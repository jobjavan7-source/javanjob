...
class JJAA_Generator {
	...
	public static function run( $overrides = array() ) {
		...
		// با تولید روزانه، پنجره‌ی ۲۰تایی بعد از سه هفته دور می‌زند و مدل
		// دوباره سراغ همان موضوع‌ها می‌رود؛ ۶۰ تا تقریباً دو ماه را پوشش
		// می‌دهد. عنوان‌های زمان‌بندی‌شده (future) هم باید بیایند، وگرنه
		// چند مقاله‌ی در صف می‌توانند هم‌موضوع از آب دربیایند.
		$avoid_titles = $topic === '' ? self::recent_titles( 60 ) : array();
		$prompt       = self::build_prompt( $avoid_titles, $topic, $word_count, $image_count );
	...
	private static function build_prompt( $avoid_titles, $topic = '', $word_count = 0, $image_count = 0 ) {
		$topic_text = $topic !== ''
		...
		// به تعداد عکسِ درخواستی عبارت جست‌وجو بخواه (حداقل ۳): اگر کمتر
		// بیاید، کد ناچار می‌شود آخرین عبارت را تکرار کند و عکس‌ها شبیه هم
		// از آب درمی‌آیند.
		$image_query_count = max( 3, (int) $image_count );
...
- ساختار: مقدمه‌ی کوتاه جذاب، چند بخش با تیتر H2 (و در صورت نیاز H3)، پاراگراف‌های کوتاه (۲ تا ۴ جمله)، حداقل یک لیست (ul/ol) در متن، و یک بخش «سؤالات متداول» در پایان با ۴ تا ۶ سؤال و پاسخ کوتاه.
- سئو: از کلمه‌ی کلیدی اصلی به‌طور طبیعی (نه تکرار زورکی) در تیتر، مقدمه، و حداقل یکی از H2ها استفاده کن.

قواعد GEO (بهینه‌سازی برای موتورهای پاسخ‌گوی هوش مصنوعی مثل ChatGPT و Gemini و پرپلکسیتی). این موتورها تکه‌تکه از متن نقل‌قول می‌کنند، نه کل صفحه را؛ پس:
- بلافاصله بعد از مقدمه، یک پاراگراف ۴۰ تا ۶۰ کلمه‌ای بنویس که مستقیم و کامل به سؤال اصلیِ عنوان مقاله جواب بدهد. این پاراگراف باید به‌تنهایی و بدون خواندن بقیه‌ی مقاله معنا بدهد.
- هر بخش H2 را هم با یک تا دو جمله‌ی پاسخ مستقیم شروع کن، بعد توضیح بده.
- هر جمله باید مستقل باشد: از ارجاع‌هایی مثل «همان‌طور که بالاتر گفتیم»، «در بخش قبل» یا «این مورد» استفاده نکن، چون وقتی یک پاراگراف جدا نقل شود بی‌معنا می‌شود. به‌جای ضمیر، خودِ اسم را تکرار کن.
- تا جای ممکن مشخص و عددی بنویس: بازه‌ی حقوق، تعداد مرحله، مدت زمان، درصد. موتورهای پاسخ‌گو جمله‌های دارای عدد و جزئیات را بیشتر نقل می‌کنند تا جمله‌های کلی.
- پاسخ‌های بخش «سؤالات متداول» را کامل و خودبسنده بنویس (۲ تا ۴ جمله)، نه یک‌کلمه‌ای.
- {$avoid_text}
...
نکته‌ی مهم درباره‌ی image_queries: چون بانک تصویر انگلیسی‌زبان است، {$image_query_count} عبارت کوتاه انگلیسی (۲ تا ۴ کلمه) بده که به‌خوبی موضوع مقاله را در آن بانک تصویری نشان دهند (مثلاً "office job interview" یا "iranian workers factory"). عبارت‌ها باید با هم فرق داشته باشند تا عکس‌های تکراری گرفته نشود.
PROMPT;
	}
